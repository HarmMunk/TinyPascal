...
137 GOSUB 8000:OPEN"R",#2,PLN$,128:FIELD#2,128 AS DST$:GOSUB 7000:REM FILE#2 STAYS OPEN
140 AC.PI=PROR:REM AC.PI IS THE 8080-CODE PROGRAM INDEX. RTS CODE STARTS AT 0100H IN CP/M
...
7004 OPEN"R",#1,RTSFNAME$,128:FIELD#1,128 AS SRC$
...
11220 RETURN
20000 REM:INTERFACE: <n> IS FILE NUMBER: 1|2|3
20010 REM: PARAMETER: FNAMEn$: FILE NAME: SET WHEN OPENING
20020 REM: INTERNAL: FBUFn$: FILE BUFFER
20030 REM: INTERNAL: RCNMn: NUMBER OF RECORD IN BUFFER: 1.. . WHEN 0, BUFFER IS EMPTY
20040 REM: INTERNAL: RCOFSn: BYTE OFFSET IN RECORD
20050 REM: INTERNAL: DRTYn: DIRTY FLAG: TRUE WHEN RECORD RCNMn HAS BEEN WRITTEN TO
20060 REM: PARAMETER: FSZn: SIZE OF FIELD (AS DEFINED BY THE APPLICATION)
20100 REM: 2n000: OPEN FILE: FNAMEn$ MUST HAVE BEEN SET
20
21000 OPEN#1,"R",FNAME1$,128:FIELD#1,FBUF1$:RCNM1=0:RCOFS1=0:DRTY1=FALSE:RETURN:FSZ1 IS THE SIZE OF A FIELD
21010 RCNM1=0:RETURN
21020 IF DIRTY1 THEN PUT#1,RCNM1
21030 CLOSE#1:RETURN
21100 TRCNM1=
